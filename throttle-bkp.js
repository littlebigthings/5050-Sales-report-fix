...
async function appendOrUpdateSheet(products) {
  ...
  const throttleSheetsRequest = async (fn, args = [], retries = 3) => {
    try {
      ...
      if (error.response && error.response.status === 429) {
        ...
        await delay(retryAfter * 1000);
        return await throttleSheetsRequest(fn, args, retries);
      } else {
        throw error;
      }
    }
  ...
  for (const product of products) {
    ...
    const existingRowsRes = await throttleSheetsRequest(sheets.spreadsheets.values.get, {
    ...
    if (rowIndex !== -1) {
      ...
      await throttleSheetsRequest(sheets.spreadsheets.values.update, {
        spreadsheetId: SHEET_ID,
        range: `Sheet1!A${targetRow}:H${targetRow}`,
      ...
      await throttleSheetsRequest(sheets.spreadsheets.values.append, {
        spreadsheetId: SHEET_ID,
